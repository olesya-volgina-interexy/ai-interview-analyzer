...
// Признак вакансии — строка вида "<Role> for <Client>" с ключевым словом роли
// (Consultant/Engineer/Developer/...), стоящая в начале блока (после пустой
// строки или в самом начале документа).
//
...
export function splitVacancies(
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Также убедимся, что выше нет ничего, кроме пустых строк, либо
    // выше есть какое-то содержимое (то есть это не первая строка тикета,
    // которая может быть пустой prefix).
    if (
...
// Нормализует ввод пользователя к идентификатору Linear:
//   - "LIN-1234"                            → "LIN-1234"
//   - "lin-1234"                            → "LIN-1234"
//   - "https://linear.app/foo/issue/LIN-1234/slug" → "LIN-1234"
//   - UUID                                  → UUID как есть
// Возвращает null, если ничего распознать не удалось.
export function normalizeIssueIdentifier(input: string): string | null {
  const trimmed = input.trim();
  if (!trimmed) return null;

  // UUID — пропускаем как есть (Linear API принимает оба формата).
  const uuidMatch = trimmed.match(
    /[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}/i
  );
  if (uuidMatch) return uuidMatch[0];

  // Team-prefix идентификатор: ABC-123, LIN-1234, ENG-42 — где-то в строке.
  const idMatch = trimmed.match(/\b([A-Z][A-Z0-9_]*-\d+)\b/i);
